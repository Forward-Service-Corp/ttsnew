...
export default function Dashboard() {
    ...
    pageData.tasks.filter(task => eval(task.completed) === true).length;
    ...
    useEffect(() => {
        fetch(`/api/getUser?email=${sessionData.user.email}`)
            .then(response => response.json())
            .then(data => setPageData(data)).then(data => console.log(data))
    }, [pageData])
