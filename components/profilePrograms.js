...
function ProfilePrograms({programs, setPrograms}) {
    ...
    return (
            <ProfileSectionStyle title={`Programs`} buttonActionTitle={"+ Add "} open={open} setOpen={setOpen}>
                <div className={`${open ? 'visible' : 'hidden'}`}>
                    <h2 className={"uppercase text-gray-600 font-light mb-3 dark:text-white"}>Available Programs</h2>
                    <select className={`h-[260px] w-full border-2 ${getFieldClass(true, programs)}`} multiple
                            onChange={(e) => {
                        ...
                        <option value={`Highway Construction Skills Training (HCST)`}>Highway Construction Skills
                            Training
                            (HCST)
                        </option>
                        ...
                        <option value={`Wisconsin Employment Transportation Assistance Program (WETAP)`}>Wisconsin
                ...
                <h2 className={"uppercase text-gray-600 font-light mb-3 mt-6 dark:text-white"}>Your Selected Programs<span className={"text-red-600"}>*</span></h2>
                    <div className={"lg:flex-1 flex flex-wrap"}>
                        {programs.map(program => (
                            <div className={"cursor-pointer rounded py-1 px-2 min-h-8 mr-2 mb-2 flex justify-between items-center text-xs text-black w-auto bg-yellow-100"} key={program}
                                 onClick={() => {
                                     setPrograms(prevState => prevState.filter(item => item !== program))
                                 }}>
                                <div className={""}>{program}</div>
                                <div className={"ml-2"}><XCircle size={20} weight="regular" color={"red"}/></div>
                            </div>
                        ))}
                    </div>
            </ProfileSectionStyle>
