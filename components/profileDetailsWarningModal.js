...
export default function ProfileDetailsWarningModal({ session }) {
  const checkRequiredFields = () => {
    if (session?.name === "") {
      ...
      return session.programs?.length === 0 || session.programs === undefined;
  };
                  ...
                  <div className="mt-3 text-center sm:mt-5">
                    ...
                    <div className="mt-2">
                      <p className="text-sm text-gray-500">
                        We require your full name, county of residence, at least
                        one county you would access services in, and the
                        program(s) you are currently enrolled in to proceed.
                      </p>
                    </div>
                  </div>
                </div>
