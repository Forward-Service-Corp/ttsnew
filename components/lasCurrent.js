    ...
    return (
        <div className={"grid grid-cols-1 lg:grid-cols-2"}>
            {surveys.sort((a, b) => new Date(b.datestamp) - new Date(a.datestamp)).map((survey, i) => {
                if(i === 0){
                    return (
                        <div className={"rounded shadow mr-5 mb-5 text-sm overflow-hidden relative flex flex-col justify-between"} key={survey._id}>
                            <div className={"bg-gray-700 p-3 truncate font-light text-white"}>{moment(survey.datestamp).format("dddd, MMMM Do YYYY, h:mm:ss a")}</div>
