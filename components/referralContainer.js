...
function ReferralContainer({item, user, notes}) {
    ...
    return (
        <div className={"my-3"} key={item._id}>
            <div className={"flex justify-start items-center text-sm bg-gradient-to-r from-orange-600 to-orange-400 font-light text-white relative p-3"}>
                ...
                <div className={"absolute right-0 min-w-[100px] flex items-center justify-between h-full pl-3  bg-gray-700"}>
            ...
            <div className={`flex ${open ? "visible" : "hidden"} flex-col md:flex-row`}>
                ...
                <div className={"w-full p-5 inline"}>
                    ...
                    <input type={"text"} className={"w-full border-0 border-b-[1px] p-1 text-sm font-light"} value={task} placeholder={"Enter new todo item..."} onChange={(e) => {
                    ...
                    {allTasks && allTasks.filter(item => eval(item.completed) === true).length > 0 ? <div className={"uppercase text-orange-600 text-sm mt-4"}>Completed</div> : null}
